...
fn main() -> std::io::Result<()> {
    ...
    let mut style_file = File::open(style_path).expect("could not open style file");
    ...
    for path in &pathes {
        if path.ends_with("README.md") {
            ...
            let internal_links_in_file = parse_internal_links(readme.as_str());
    ...
    for path in pathes {
        let stripped_path = path
            ...
            .expect(format!("could not strip the in_path prefix: {:?}", in_path).as_str());
        ...
        if stripped_path.ends_with("README.md") {
            ...
            let raw_path = ancestors.next().expect("could not extract next ancestor");
            ...
            write_readme_content(&mut file, in_path.as_path(), raw_path, &internal_links)?;
...
fn write_nav(
    ...
    if analytics == true {
        file.write_all(
            format!(
                r#"<img style="display: none" src="https://stats.emile.space/count?p=/{}">
        <nav>
          <ul>"#,
  ...
  <nav>
    ...
    for component in slice {
        ...
        // string
        let nav_breadcrumb_link_absolute = Path::new("/").join(nav_breadcrumb_link);

        let nav_breadcrumb_link = nav_breadcrumb_link_absolute.to_str().unwrap();
        ...
        <li>
            <a href="{}">{}</a>
            <ul>"#,
                nav_breadcrumb_link, nav_breadcrumb_name
            )
            .as_bytes(),
        )?;
        ////////////////////////////////////////////////////////////////////////
        ...
        for dir in dirs {
            ...
            let rel_link = d
                .strip_prefix(abs_inpath)
                .expect(format!("could not strip the in_path prefix: {:?}", d).as_str());
...
fn write_same_level(file: &mut File, in_path: &Path, raw_path: &Path) -> std::io::Result<()> {
  ...
  <ul>"#
    ...
    <ul>"#
        ...
        for f in files {
            ...
            if name == "README.md" || name == "show_files" || name.starts_with(".") {
                continue;
            };
...
fn write_readme_content(
    ...
    let readme_file_path = Path::new(in_path).join(raw_path).join("README.md");
...
"##,
        ...
        if line.starts_with("---") {
        ...
        } else if line.starts_with(":::backlinks") {
            // Backlinks
            ...
            file.write_all(r#"</pre><pre class="backlinks">Backlinks<br>"#.as_bytes())?;

            match internal_links.get(&search_path) {
                Some(values) => {
                    // only write "backlinks" if we've actually got some
                    // file.write_all(b"<pre>backlinks:</pre>\n")?;

                    for link in values {
                        // strip the "in" prefix
                        // strip the "README.md" suffix
                        // TODO: do all this magic by parsing it as a path and removing the unneeded parts,
                        //       bonus by creating a function doing this and removing the horrible string
                        //       mashing in this codebase
                        let a = link
                            .strip_prefix("in")
...
"#).as_bytes())?;
            ...
            let mut tree_files = recursive_read_dir(&tree_files_path, true)?;
            ...
            // wrapping the whole toc in a .toc class, escaping the pre
            // TODO(emile): the whole pre escaping thing is super ugly, actually parse this, as the
            // current state is just not going to work well should I ever change more stuff in here
            // (which I hope I don't)
            file.write_all(r#"</pre><pre class="toc">Table of Contents<br>"#.as_bytes())?;
...
"##,
            ...
            file.write_all("</pre><pre>".as_bytes())?;
        } else {
...
// <a rel="me" href="https://chaos.social/@hanemile" target="_blank" class="icon"><img class="webring" src="/mastodon.svg" alt="mastodon" height="0px" style="display: none"/></a>
...
fn gitignore_entries(dir: &PathBuf) -> io::Result<Vec<PathBuf>> {
    let gitignore_path = Path::new(&dir).join(Path::new(".gitignore"));
