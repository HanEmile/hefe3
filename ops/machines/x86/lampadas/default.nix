...
in
{
  ...
  boot = {
    ...
    # Swap the in-tree r8169 driver for the out-of-tree r8168. r8169
    # on this RTL8168h/8111h NIC racks up thousands of
    #   "NETDEV WATCHDOG: transmit queue 0 timed out"
    #   "rtl_txcfg_empty_cond == 0"
    # events per day (~6-11k/day observed late May 2026), keeping the
    # NIC in a near-continuous TX timeout / reset cycle. The vendor
    # driver r8168 doesn't hit this on the same hardware.
    ...
    # culprits on RTL8168h/8111h with the vendor driver are PCIe ASPM
    # power management and Energy Efficient Ethernet (EEE). Both are
    # known to wedge the TX queue under load and are routinely disabled
    # in the Realtek vendor driver READMEs. eee_enable=0 also matches
    # the "Advertised EEE link modes" the NIC was offering before this
    # change.
    extraModprobeConfig = ''
      options r8168 aspm=0 eee_enable=0 s0_magic_packet=0
    '';
    ...
    initrd = {
      ...
      # the .ko into the initrd module search path; kernelModules here
      # force-loads it before network comes up.
      availableKernelModules = [ "r8168" ];
      kernelModules = [ "r8168" ];
  ...
  services = {
    ...
    restic.backups."photos" = {
      ...
      timerConfig = {
        OnCalendar = "daily";
        Persistent = true;
        # Spread firing time so a flaky NIC / CIFS link does not always
        # collide with the exact same wall-clock minute. Matches the
        # standard set by ops/vms/x86/modules/backups.nix.
        RandomizedDelaySec = "1h";
      };
      # Wait for the storagebox CIFS automount to actually be there
      # before restic starts. Previously restic raced the mount and
      # spent 15 min retrying with "no such device" errors.
    ...
    # Immich library at /data/private/immich (~430 GB after excludes).
    # Separate repo from "photos" because the data lives on a different
    # filesystem and the source path differs - keeping one repo per
    # source path keeps restic's snapshot history clean and lets us
    # forget/prune each independently.
    #
    # TODO(emile): add a second copy of this repo on medano grave/restic
    #   (e.g. via `restic copy` from bx11 -> medano scheduled on medano,
    #   or a direct sftp:root@medano.pinto-pike.ts.net target). Skipped
    #   for now while we wait for the initial ~430 GB seed to bx11.
    #
    # Excludes:
    #   - thumbs: regenerated by Immich on demand
    #   - encoded-video: ditto
    #   - upload: ephemeral staging dir, files move into library once processed
    restic.backups."immich-data" = {
      timerConfig = {
        OnCalendar = "daily";
        Persistent = true;
        RandomizedDelaySec = "1h";
      ...
      backupPrepareCommand = "${pkgs.util-linux}/bin/findmnt /mnt/storagebox-bx11 > /dev/null || ls /mnt/storagebox-bx11 > /dev/null";
      repository = "/mnt/storagebox-bx11/backup/immich-data";
      initialize = true;
      passwordFile = config.age.secrets."storagebox_bx11_restic_password".path;

      paths = [ "/data/private/immich" ];
      exclude = [
        "/data/private/immich/thumbs"
        "/data/private/immich/encoded-video"
        "/data/private/immich/upload"
      ];
      pruneOpts = [
        "--keep-daily 7"
        "--keep-weekly 5"
  ...
  fileSystems."/mnt/storagebox-bx11" = {
    ...
    options =
      ...
      in
      [
        "_netdev,x-systemd.automount,noauto,x-systemd.idle-timeout=60s,x-systemd.mount-timeout=5s,credentials=${conn_config}"
      ];
  };
  ...
  # Disabling segmentation/large-receive offloads is the standard
  # mitigation for this class of Realtek TX-timeout bug (same pattern as
  # the medano e1000e fix in ops/machines/x86/medano/default.nix).
  # Both restic-backups-* units run with PrivateTmp + RuntimeDirectory +
  # CacheDirectory, which causes systemd to allocate them a private mount
  # namespace. The /mnt/storagebox-bx11 autofs trigger is set up in the
  # host namespace, but the underlying CIFS mount never propagates into
  # the unit's namespace - so accessing /mnt/storagebox-bx11/backup/...
  # fails with ENODEV ("no such device") and restic gives up with
  # "unable to open repository / mkdir /mnt/storagebox-bx11/backup: no
  # such device" (observed 2026-06-01 for immich-data: 15 min of retry
  # loops then failure, even though the share is mounted fine on the
  # host). RequiresMountsFor activates the automount AND tells systemd
  # to include /mnt/storagebox-bx11 in the unit's mount-namespace
